...
if (!empty($id) && isset($_POST['Modelo'])) {
    ...
    $modelo = $conn->real_escape_string($_POST['Modelo']); // Escapa caracteres especiais para evitar SQL Injection

    // Converter o modelo para maiúsculas
    $modelo = strtoupper($modelo);

    // Construir a consulta SQL para atualização (usando prepared statement para segurança adicional)
    $sql = "UPDATE MODELO SET MODELO=? WHERE IDMODELO=?"; // SQL para atualizar o valor do modelo baseado no ID
    ...
    // Se o ID estiver vazio ou o campo Modelo não for enviado, redirecionar para a página de falha e registrar o erro
    error_log("ID vazio ou campo Modelo não enviado" . $stmt->error); // Registra a falha no log de erros
    header("Location: ../ViewFail/FailCreateModificaModelo.php?erro=" . urlencode("Não foi possível realizar a alteração no cadastro do modelo. Refaça a operação e tente novamente"));
    exit(); // Interrompe a execução do script após o redirecionamento
}
