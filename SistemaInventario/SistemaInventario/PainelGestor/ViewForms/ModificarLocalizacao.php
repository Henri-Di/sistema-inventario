...
<head>
    ...
    <!-- Title page -->
    <title>Sistema de Inventário - Modificar Localização</title>
...
<div class="container-fluid" style="margin-left:25%;">
    <table class="table table-bordered" style="width:60%;height:auto;">
        <form method="POST" class="example" action="../ViewFunctions/CreateModificaLocalizacao.php">
            ...
            <?php 
                ...
                $con = mysqli_query($conn, $consulta) or die(mysqli_error($conn));
            ?> 
            <!-- Fim do código PHP para listagem dos material -->

            <!-- Início do código PHP para repetição de listagem -->
            <?php while($dado = $con->fetch_array()) { ?> 
                    <td id="colun-blue-table">   
                        <div id="blue-title-listar">Localização</div>
                        <input type="text" id="blue-input-cdst" value="<?php echo $dado['LOCALIZACAO']; ?>" name="Localizacao"/>
                    </td>
                </tr>
            <?php } ?>
            <!-- Fim do código PHP para repetição de listagem -->
    ...
    <button type="submit" id="blue-button-nova-senha">Modificar Localização <i class="fa fa-pencil"></i></button>
    </form>
</div>
