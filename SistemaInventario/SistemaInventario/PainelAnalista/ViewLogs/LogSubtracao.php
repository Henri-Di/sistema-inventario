...
<head>
    ...
    <!-- Title page -->
    <title>Sistema de Inventário - Log Diminuição</title>
...
<div id="blue-line-title-btn-painel">
    <p id="blue-title-btn-painel">Log Diminuição <i class="fa fa-level-down" id="blue-icon-btn-painel"></i></p>
</div>

<div class="container-fluid" style="width: 100%; height: 50%; overflow-y: auto; overflow-x: hidden; margin-top: 1%;">
    <br>

    <!-- Início do código PHP para consultar dados de diminuição -->
    <?php
    require_once('../../ViewConnection/ConnectionInventario.php');

    // Obtém o ID do produto da URL e filtra como número inteiro
    $id = filter_input(INPUT_GET, 'id', FILTER_SANITIZE_NUMBER_INT);

    $consulta = "SELECT * FROM SUBTRACAO WHERE IDPRODUTO='$id'";
    $con = mysqli_query($conn, $consulta) or die(mysqli_error($conn));
    ?>
    <!-- Fim do código PHP para consultar dados de diminuição -->

    <!-- Início do loop PHP para exibir dados de diminuição -->
    <?php while ($dado = $con->fetch_array()) { ?>

        <!-- Início do código PHP para conversão de data para formato brasileiro -->
        <?php
        $dateSubtracao = strtotime($dado['DATASUBTRACAO']);
        $dateformatedSubtracao = date("d/m/Y", $dateSubtracao);
        ?>
        <!-- Fim do código PHP para conversão de data para formato brasileiro -->

        <table class="table table-bordered" id="blue-table-cadastro-auxiliar">
            <tr id="line-blue-table-hover">
                <form method="POST" action="">
                    <td id="colun-blue-table">
                        <div id="blue-title-listar">Código Diminuição</div>
                        <input type="text" id="blue-input-cdst" name="CodigoDiminuicao" value="<?php echo $dado['ID']; ?> " autocomplete="off" required disabled /><br>
                    </td>

                    <td id="colun-blue-table">
                        <div id="blue-title-listar">Nº WO</div>
                        <input type="text" id="blue-input-cdst" name="NumWo" value="<?php echo $dado['NUMWO']; ?> " autocomplete="off" required disabled /><br>
                    </td>

                    <td id="colun-blue-table">
                        <div id="blue-title-listar">Quantidade Diminuída</div>
                        <input type="text" id="blue-input-cdst" name="QuantidadeSubtraida" value="<?php echo $dado['QUANTIDADE']; ?> " autocomplete="off" required disabled /><br>
                    </td>

                    <td id="colun-blue-table">
                        <div id="blue-title-listar">Data Registro</div>
                        <input type="text" id="blue-input-cdst" name="DataSubtracao" value="<?php echo $dateformatedSubtracao; ?> " autocomplete="off" required disabled /><br>
                    </td>

                    <td id="colun-blue-table">
                        <div id="blue-title-listar">Observações</div>
                        <input type="text" id="blue-input-cdst" name="ObservacaoSubtracao" value="<?php echo $dado['OBSERVACAO']; ?> " autocomplete="off" required disabled /><br>
                    </td>
                    ...
                    <td id="colun-blue-table">
                        <div id="blue-title-listar">Analista</div>
                    ...
                    <td id="colun-blue-table">
                        ...
                        <input type="text" id="blue-input-cdst" name="CódigoAnalista" value="<?php echo $dado['CODIGOP']; ?>" autocomplete="off" required disabled /><br>
                    </td>
                </form>
            </tr>
        </table>
    ...
    <?php } ?>
    <!-- Fim do loop PHP para exibir dados de diminuição -->
</div>
