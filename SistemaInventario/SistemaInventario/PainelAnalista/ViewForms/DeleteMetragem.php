...
<head>
    ...
    <!-- Title page -->
    <title>Sistema de Inventário - Remover Metragem</title>
...
<div class="container-fluid">
    ...
    <p id="blue-title-btn-painel">Remover Metragem <i class="fa fa-trash" id="blue-icon-btn-painel"></i></p>
</div>
<br>
<div class="container-fluid" style="margin-left:25%;">
    <table class="table table-bordered" style="width:60%;height:auto;">
        <form method="POST" class="example" action="">
            ...
            <!-- Início do código PHP para listagem das metragens -->
            <?php 
                require_once('../../ViewConnection/ConnectionInventario.php'); 
                $id = filter_input(INPUT_GET, 'id', FILTER_SANITIZE_NUMBER_INT);
                $consulta = "SELECT * FROM METRAGEM WHERE IDMETRAGEM='$id'";
                $con = mysqli_query($conn, $consulta) or die(mysqli_error($conn, $consulta));
            ?> 
            <!-- Fim do código PHP para listagem das metragens -->
            ...
            <?php while($dado = $con->fetch_array()) { ?> 
                <tr id="line-blue-table">
                    <td id="colun-blue-table">   
                        ...
                        <input type="text" id="blue-input-cdst" value="<?php echo $dado['IDMETRAGEM']; ?>" name="id" disabled>
                        <input type="hidden" id="blue-input-cdst" value="<?php echo $dado['IDMETRAGEM']; ?>" name="id">
                    </td>
            <?php } ?>
            ...
            <?php 
                ...
                $con = mysqli_query($conn, $consulta) or die(mysqli_error($conn, $consulta));
            ?> 
            <!-- Fim do código PHP para listagem dos metragens -->

            <!-- Início do código PHP para repetição de listagem -->
            <?php while($dado = $con->fetch_array()) { ?> 
                    <td id="colun-blue-table">   
                        <div id="blue-title-listar">Metragem</div>
                        <input type="text" id="blue-input-cdst" value="<?php echo $dado['METRAGEM']; ?>" name="Metragem" disabled/>
                        <input type="hidden" id="blue-input-cdst" value="<?php echo $dado['METRAGEM']; ?>" name="Metragem"/>
                    </td>
                </tr>
            <?php } ?>
            <!-- Fim do código PHP para repetição de listagem -->
    ...
    <button type="submit" id="blue-button-nova-senha" onclick="window.location.href='../ViewFunctions/CreateDeleteMetragem.php?id=<?php echo $dado['IDMETRAGEM'];?>';">Remover Metragem <i class="fa fa-trash"></i></button>
    </form>
</div>
