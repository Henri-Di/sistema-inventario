...
if (!empty($id) && isset($_POST['Conector'])) {
    // Sanitizar o valor do conector usando real_escape_string para prevenir injeção de SQL
    $conector = $conn->real_escape_string($_POST['Conector']); // Escapa caracteres especiais para evitar SQL Injection

    // Converter o conector para maiúsculas
    $conector = strtoupper($conector);

    // Construir a consulta SQL para atualização (usando prepared statement para segurança adicional)
    $sql = "UPDATE CONECTOR SET CONECTOR=? WHERE IDCONECTOR=?"; // SQL para atualizar o valor do conector baseado no ID
    $stmt = $conn->prepare($sql); // Prepara a consulta SQL
    $stmt->bind_param("si", $conector, $id); // Associa os parâmetros à consulta SQL: o primeiro parâmetro é o valor do conector e o segundo é o ID

    // Executar a consulta preparada
    if ($stmt->execute()) {
        // Se a execução da consulta for bem-sucedida, redirecionar para a página de sucesso
        header("Location: ../ViewSucess/SucessCreateModificaConector.php?sucesso=" . urlencode("A alteração foi realizada com sucesso no cadastro do conector"));
        ...
        error_log("Erro na atualização do conector: " . $stmt->error); // Registra o erro no log de erros
    ...
    // Se o ID estiver vazio ou o campo Conector não for enviado, redirecionar para a página de falha e registrar o erro
    error_log("ID vazio ou campo Conector não enviado" . $stmt->error); // Registra a falha no log de erros
    header("Location: ../ViewFail/FailCreateModificaConector.php?erro=" . urlencode("Não foi possível realizar a alteração no cadastro do conector. Refaça a operação e tente novamente"));
    exit(); // Interrompe a execução do script após o redirecionamento
}
