...
if (!empty($id) && isset($_POST['Localizacao'])) {
    // Sanitizar o valor da localizacao usando real_escape_string para prevenir injeção de SQL
    $localizacao = $conn->real_escape_string($_POST['Localizacao']); // Escapa caracteres especiais para evitar SQL Injection

    // Converter a localizacação para maiúsculas
    ...
    $sql = "UPDATE LOCALIZACAO SET LOCALIZACAO=? WHERE IDLOCALIZACAO=?"; // SQL para atualizar o valor da localização baseado no ID
    $stmt = $conn->prepare($sql); // Prepara a consulta SQL
    $stmt->bind_param("si", $localizacao, $id); // Associa os parâmetros à consulta SQL: o primeiro parâmetro é o valor da localização e o segundo é o ID

    // Executar a consulta preparada
    if ($stmt->execute()) {
        // Se a execução da consulta for bem-sucedida, redirecionar para a página de sucesso
        header("Location: ../ViewSucess/SucessCreateModificaLocalizacao.php?sucesso=" . urlencode("A alteração foi realizada com sucesso no cadastro da localização"));
        ...
        error_log("Erro na atualização da localização: " . $stmt->error); // Registra o erro no log de erros
    ...
    // Se o ID estiver vazio ou o campo Localização não for enviado, redirecionar para a página de falha e registrar o erro
    error_log("ID vazio ou campo Localização não enviado" . $stmt->error); // Registra a falha no log de erros
    header("Location: ../ViewFail/FailCreateModificaLocalizacao.php?erro=" . urlencode("Não foi possível realizar a alteração no cadastro da localização. Refaça a operação e tente novamente"));
    exit(); // Interrompe a execução do script após o redirecionamento
}
