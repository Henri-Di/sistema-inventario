...
if (!empty($id) && isset($_POST['Material'])) {
    // Sanitizar o valor do material usando real_escape_string para prevenir injeção de SQL
    $material = $conn->real_escape_string($_POST['Material']); // Escapa caracteres especiais para evitar SQL Injection

    // Converter o material para maiúsculas
    $material = strtoupper($material);

    // Construir a consulta SQL para atualização (usando prepared statement para segurança adicional)
    $sql = "UPDATE MATERIAL SET MATERIAL=? WHERE IDMATERIAL=?"; // SQL para atualizar o valor do material baseado no ID
    $stmt = $conn->prepare($sql); // Prepara a consulta SQL
    $stmt->bind_param("si", $material, $id); // Associa os parâmetros à consulta SQL: o primeiro parâmetro é o valor do material e o segundo é o ID
    ...
    if ($stmt->execute()) {
        ...
        error_log("Erro na atualização do material: " . $stmt->error); // Registra o erro no log de erros
    ...
    // Se o ID estiver vazio ou o campo Material não for enviado, redirecionar para a página de falha e registrar o erro
    error_log("ID vazio ou campo Material não enviado" . $stmt->error); // Registra a falha no log de erros
    header("Location: ../ViewFail/FailCreateModificaMaterial.php?erro=" . urlencode("Não foi possível realizar a alteração no cadastro do material. Refaça a operação e tente novamente"));
    exit(); // Interrompe a execução do script após o redirecionamento
}
