...
if (!empty($id) && isset($_POST['Fornecedor'])) {
    ...
    // Converter o fornecedor para maiúsculas
    ...
    $sql = "UPDATE FORNECEDOR SET FORNECEDOR=? WHERE IDFORNECEDOR=?"; // SQL para atualizar o valor do fornecedor baseado no ID
    ...
    if ($stmt->execute()) {
        ...
        header("Location: ../ViewSucess/SucessCreateModificaFornecedor.php?sucesso=" . urlencode("A alteração foi realizada com sucesso no cadastro do fornecedor"));
        ...
        error_log("Erro na atualização do fornecedor: " . $stmt->error); // Registra o erro no log de erros
    ...
    // Se o ID estiver vazio ou o campo Fornecedor não for enviado, redirecionar para a página de falha e registrar o erro
    error_log("ID vazio ou campo Fornecedor não enviado: "  . $stmt->error); // Registra a falha no log de erros
    header("Location: ../ViewFail/FailCreateModificaFornecedor.php?erro=" . urlencode("Não foi possível realizar a alteração no cadastro do fornecedor. Refaça a operação e tente novamente"));
    exit(); // Interrompe a execução do script após o redirecionamento
}
