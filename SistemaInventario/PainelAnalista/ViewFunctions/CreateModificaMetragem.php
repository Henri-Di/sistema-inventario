...
if (!empty($id) && isset($_POST['Metragem'])) {
    // Sanitizar o valor do metragem usando real_escape_string para prevenir injeção de SQL
    $metragem = $conn->real_escape_string($_POST['Metragem']); // Escapa caracteres especiais para evitar SQL Injection

    // Converter a metragem para maiúsculas
    $metragem = strtoupper($metragem);

    // Construir a consulta SQL para atualização (usando prepared statement para segurança adicional)
    $sql = "UPDATE METRAGEM SET METRAGEM=? WHERE IDMETRAGEM=?"; // SQL para atualizar o valor da metragem baseado no ID
    ...
    // Se o ID estiver vazio ou o campo Metragem não for enviado, redirecionar para a página de falha e registrar o erro
     // Registra a falha no log de erros
    header("Location: ../ViewFail/FailCreateModificaMetragem.php?erro=" . urlencode("Não foi possível realizar a alteração no cadastro da metragem. Refaça a operação e tente novamente"));
    exit(); // Interrompe a execução do script após o redirecionamento
}
