...
<head>
    ...
    <!-- Title page -->
    <title>Sistema de Inventário - Remover Conector</title>
...
<div class="container-fluid">
    ...
    <p id="blue-title-btn-painel">Remover Conector <i class="fa fa-trash" id="blue-icon-btn-painel"></i></p>
</div>
<br>
<div class="container-fluid" style="margin-left:25%;">
    <table class="table table-bordered" style="width:60%;height:auto;">
        <form method="POST" class="example" action="">
            ...
            <!-- Início do código PHP para listagem dos conectores -->
            <?php 
                require_once('../../ViewConnection/ConnectionInventario.php'); 
                $id = filter_input(INPUT_GET, 'id', FILTER_SANITIZE_NUMBER_INT);
                $consulta = "SELECT * FROM CONECTOR WHERE IDCONECTOR='$id'";
                $con = mysqli_query($conn, $consulta) or die(mysqli_error($conn, $consulta));
            ?> 
            <!-- Fim do código PHP para listagem dos conectores -->
            ...
            <?php while($dado = $con->fetch_array()) { ?> 
                <tr id="line-blue-table">
                    <td id="colun-blue-table">   
                        ...
                        <input type="text" id="blue-input-cdst" value="<?php echo $dado['IDCONECTOR']; ?>" name="id" disabled>
                        <input type="hidden" id="blue-input-cdst" value="<?php echo $dado['IDCONECTOR']; ?>" name="id">
                    </td>
            <?php } ?>
            ...
            <?php while($dado = $con->fetch_array()) { ?> 
                    <td id="colun-blue-table">   
                        ...
                        <input type="text" id="blue-input-cdst" value="<?php echo $dado['CONECTOR']; ?>" name="Conector" disabled/>
                        <input type="hidden" id="blue-input-cdst" value="<?php echo $dado['CONECTOR']; ?>" name="Conector"/>
                    </td>
                </tr>
            <?php } ?>
            <!-- Fim do código PHP para repetição de listagem -->
    ...
    <button type="submit" id="blue-button-nova-senha" onclick="window.location.href='../ViewFunctions/CreateDeleteConector.php?id=<?php echo $dado['IDCONECTOR'];?>';">Remover Conector <i class="fa fa-trash"></i></button>
    </form>
</div>
